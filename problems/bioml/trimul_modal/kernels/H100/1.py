...
def two_mm_kernel(a_ptr, b1_ptr, b2_ptr, b3_ptr, b4_ptr, b5_ptr, c1_ptr, c2_ptr, d_ptr, mask_ptr, M, N, K, stride_a0, stride_a1, stride_a2, stride_a3, stride_bk, stride_bn, stride_c0, stride_c1, stride_c2, stride_c3, seq_len, stride_d0, stride_d1, stride_d2, stride_d3, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, BLOCK_K: tl.constexpr, GROUP_SIZE_M: tl.constexpr, NUM_SMS: tl.constexpr):
    ...
    # @xh: `_experimental_make_tensor_descriptor` -> `make_tensor_descriptor`
    ...
    #     block_shape=[BLOCK_N, BLOCK_K],
    # )
    # b3_desc = tl.make_tensor_descriptor(
    #     b3_ptr,
    #     shape=[N, K],
    #     strides=[stride_bn, stride_bk],
    #     block_shape=[BLOCK_N, BLOCK_K],
    # )
    # b4_desc = tl.make_tensor_descriptor(
    #     b4_ptr,
    #     shape=[N, K],
    #     strides=[stride_bn, stride_bk],
    #     block_shape=[BLOCK_N, BLOCK_K],
    # )
    # b5_desc = tl.make_tensor_descriptor(
    #     b5_ptr,
    #     shape=[N, K],
    #     strides=[stride_bn, stride_bk],
    #     block_shape=[BLOCK_N, BLOCK_K],
...
def custom_kernel(data: input_t) -> output_t:
    ...
    left, right, out_gate = two_mm(x, weights["left_proj.weight"].to(torch.float16), weights["right_proj.weight"].to(torch.float16), weights["left_gate.weight"].to(torch.float16), weights["right_gate.weight"].to(torch.float16), weights["out_gate.weight"].to(torch.float16), mask)
