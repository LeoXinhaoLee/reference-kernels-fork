...
def two_mm(A, left_proj, right_proj, left_gate, right_gate, out_gate, mask):
    ...
    if torch.cuda.get_device_capability()[0] == 10 and False:
        # Get optimal configuration for B200
        BLOCK_M, BLOCK_N, BLOCK_K, num_stages, num_warps = get_optimal_two_mm_config(B, seq_len, K)
        grid_size = min(NUM_SMS, triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N))

        two_mm_kernel[(grid_size,)](
        ...
        BLOCK_M, BLOCK_N, BLOCK_K, num_stages, num_warps = get_optimal_two_mm_config_h100(B, seq_len, K)
        grid_size = min(NUM_SMS, triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N))
